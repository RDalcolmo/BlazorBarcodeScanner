...
internal class BarcodeReaderInterop
{
    ...
    public ValueTask<List<VideoInputDevice>> GetVideoInputDevices(string message)
    {
        ...
        return _jsRuntime.InvokeAsync<List<VideoInputDevice>>(
    ...
    public async Task StartDecoding(ElementReference video)
    {
        ...
        {
            if (e.Message.IndexOf("Permission denied", StringComparison.OrdinalIgnoreCase) > -1 || e.Message.IndexOf("The request is not allowed by the user agent", StringComparison.OrdinalIgnoreCase) > -1 )
            {
    ...
    {
        return await _jsRuntime.InvokeAsync<string>("BlazorBarcodeScanner.getSelectedDeviceId");
    }
